...
def sample(click_id: int, offer_ids: str) -> dict:
    ...
    ucb_values = [
        (offer_rewards.get(offer, 0) / max(offer_clicks.get(offer, 0), 1)) +
        c * np.log1p(sum(offer_clicks.values()) + 1 / max(offer_clicks.get(offer, 0), 1))
        for offer in offer_ids
    ]

    offer_id = offers_ids[ucb_values.index(max(ucb_values))]
