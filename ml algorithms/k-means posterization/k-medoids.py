...
class ImageKMedoids:
    ...
    def _assign_centroids(self, X: np.ndarray) -> np.ndarray:
        ...
        for i in range(len(X)):
            distances = cdist(X[i].reshape(1, -1), self.centroids_, metric='cityblock')
    ...
    def _update_centroids(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        for i in range(self.n_clusters):
            #self.centroids_ = self.init.copy()
            cluster_samples = X[y == i]
            cluster_distances = cdist(cluster_samples, cluster_samples, metric='cityblock')
            medoid_index = np.argmin(np.sum(cluster_distances, axis=1))
            self.centroids_[i] = cluster_samples[medoid_index]
